...
class BoardWidget extends Pad  {
    ...
    public synchronized void paintComponent(Graphics2D g) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            g.drawLine(0, SQDIM * i, SQDIM * SIDE, SQDIM * i);
        }

        g.setColor(SELECTED_COLOR);
        g.fillRect((_selectedCol - 'a') * SQDIM,
                Math.abs(_selectedRow - '7') * SQDIM, SQDIM, SQDIM);
        for (char c = 'a'; c <= 'g'; c++) {
            for (char r = '1'; r <= '7'; r++) {
                if (_model.get(c, r) == BLOCKED) {
                    drawBlock(g,  (c - 'a') * SQDIM, Math.abs(r - '7') * SQDIM);
                } else if (_model.get(c, r) == BLUE) {
                    g.setColor(BLUE_COLOR);
                    g.fillOval((c - 'a') * SQDIM,
                            Math.abs(r - '7') * SQDIM, SQDIM, SQDIM);
        ...
        if (where.getButton() == MouseEvent.BUTTON1) {
            ...
            if (mouseCol >= 'a' && mouseCol <= 'g'
                ...
                if (_blockMode) {
                    ...
                    if (_selectedCol != 0) {
                        if (_previousSelectedCol != 0
                                && _model.legalMove(_previousSelectedCol,
                                        _previousSelectedRow, _selectedCol,
                                _selectedRow)) {
                            _commandQueue.offer("" + _previousSelectedCol
                                    + _previousSelectedRow + "-"
                                    + _selectedCol + _selectedRow);
                            _previousSelectedRow = _previousSelectedCol = 0;
                        }
                        if (_model.get(_selectedCol, _selectedRow)
                                == _model.whoseMove()) {
                            _previousSelectedCol = _selectedCol;
    ...
    /** Store the coordinates of the previously selected square. */
    private char _previousSelectedCol, _previousSelectedRow;
